    ...
    <div>
      ...
      <div className="card" style={{marginBottom:16,padding:'14px 14px 8px'}}>
        <div className="chart-container" style={{height:280}}>
          {tab==='today'&&todayProfile.length>0&&<ResponsiveContainer><ComposedChart data={todayProfile}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
          ...
          {tab==='weather'&&(weather?<ResponsiveContainer><AreaChart data={weather.map(w=>({hour:`${w.hour}h`,cloud:w.cloudCover}))}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
          ...
          {tab==='compare'&&todayProfile.length>0&&<ResponsiveContainer><AreaChart data={todayProfile}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
            <XAxis dataKey="hour" tick={{fontSize:10,fill:'var(--text-dim)'}} interval={2}/><YAxis tick={{fontSize:10,fill:'var(--text-dim)'}}/>
            <Tooltip contentStyle={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:8}}/><Legend wrapperStyle={{fontSize:11}}/>
          ...
          {tab==='monthly'&&(monthly.length>0?<ResponsiveContainer><BarChart data={monthly.slice().reverse()}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
